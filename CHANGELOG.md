...
### Fixed
- **Flaky `MapViewModelTest` on CI** — the test built fresh `MapViewModel`s but never cleared them, so their `viewModelScope` collector coroutines (location, favorites, route-simulation flows) outlived each test and kept running on the shared main test dispatcher. An exception from such a leaked coroutine then surfaced against the *next* test as `UncaughtExceptionsBeforeTest` (e.g. `toggleFavorite adds then removes favorite`), failing the suite intermittently. The test now tracks every view-model it creates and **clears each one in `tearDown`**, cancelling its scope so nothing leaks across tests.
