...
### Fixed
- **No profile starts the route on the sidewalk anymore** — the "don't snap the start/end onto a `footway`/`sidewalk`" guard (`check_start_way` + `noStartWay=footway,sidewalk`) was only present in the *trekking* and *gravel* profiles, so routing with *fastbike*, *mtb* or *shortest* still opened on the pavement with a crossing detour. The guard is now declared in **all five** bundled profiles (`PROFILES_VERSION` 5 → 6), and a regression test (`BRouterNoStartWayProbeTest`) asserts every profile populates BRouter's `noStartWays` — both on a bare parse and via the elevation slider's `uphill_extra` path.
