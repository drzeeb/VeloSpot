...
### Changed
- **Faster navigation start-up tilt-in (input locked during the glide)** — when navigation begins, the camera's "tilt-in" into the 3D view now eases in noticeably **faster** (zoom/pitch time constants scaled by 0.4 during the intro), and **all map gestures are temporarily disabled** while it plays so a stray touch can't fight the animation. Gestures (and the pan-to-unlock follow behaviour) are restored the instant the camera reaches the 3D navigation view, and always on navigation end (`NavigationManager` intro phase).
